...
describe('TC000X_WebdriverIO-Validation_Action_OnSelectors', () => {
	...
	beforeEach(async () => {
		...
		TestIdentification.setTmsLink('TC000X_WebdriverIO-Validation_Action_OnSelectors');
	...
	it('WebdriverIO-Validation - Navigate to systelab components website, find an Input Field by Id, set a new value and get of the value', async () => {
		...
		await ReportUtility.addExpectedResult('The systelab components website is open and active', async() => {
		...
		await ReportUtility.addExpectedResult('When setting a new value in the systelab component, it is found correctly by Id and set value are correct', async() => {
			AssertionUtility.expectEqual(await showCasePage.getTextBoxById().getText(), foundByID);
		});
	...
	it('WebdriverIO-Validation - Navigate to systelab components website, find an Input Field by Id, set and clear a new value', async () => {
		...
		await showCasePage.getTextBoxById().setText(foundByID);
		await ReportUtility.addExpectedResult('When setting a new value in the systelab components website, it is found correctly by Id and set value are correct', async() => {
			AssertionUtility.expectEqual(await showCasePage.getTextBoxById().getText(), foundByID);
		});
		await showCasePage.getTextBoxById().clear();
		await ReportUtility.addExpectedResult('When setting a new value in the systelab component website, clear of the value are correct', async() => {
			AssertionUtility.expectEqual(await showCasePage.getTextBoxById().getText(), empty);
		});
	...
	it('WebdriverIO-Validation - Navigate to systelab components website, find an Input Field by Id and click in button', async () => {
		...
		await ReportUtility.addExpectedResult('When setting a click on a button of the systelab component website, an expected modal window opens', async() => {
	...
	it('WebdriverIO-Validation - Navigate to systelab components website, find an Input Field by Id and identifies an attribute from that field', async () => {
		...
		await ReportUtility.addExpectedResult('The systelab components website is open and active', async() => {
			...
			await ReportUtility.addExpectedResult('An attribute is identified on a button of the systelab component web site', async() => {
			AssertionUtility.expectEqual(await showCasePage.getTextBoxDisabledById().getElement().getAttribute(attribute), attributeValue);
			});
